...
void  setup() {
  Serial.begin(115200);
  SerialBT.begin("irq_BT"); //Bluetooth device name
...
void Ang_To_Pos()
{
  /////// left left left left left
            ...
            // rear 
            u = 2;
            s_ang[u] = ((int)((hip_footyz_ang[u] * rad_deg) * -rls_as) + rlsn);

            h_ang[u] = ((int)((hip_ang[u] * rad_deg) * -rlh_as) + rlhn);

            k_ang[u] = ((int)(((180.0 - (knee_ang[u] * rad_deg)) *rlk_as) + rlkn));

            ///// right right right right right
            u = 3;
            s_ang[u] = -((int)((hip_footyz_ang[u] * rad_deg) * -rrs_as) + rrsn);

            h_ang[u] = -((int)((hip_ang[u] * rad_deg) * -rrh_as) + rrhn);
            
            k_ang[u] = -((int)(((180.0 - (knee_ang[u] * rad_deg)) * rrk_as) + rrkn));
  ...
  else
  {
    ...
    //leg = 0;
    {
        if ((leg_phase[leg] / (double)walk_cnt_max)< TOG )
        {
            ...
            footx[leg] = (int)(Stride_length_a[leg]*0.5 * cos(cycle)) + (int)Stride_offset;
        ...
        else
        {
            ...
            footx[leg] = (int)(-Stride_length_a[leg]  * (cycle-0.5)) + (int)Stride_offset;
...
if (mov)
{
  ...
  delayMicroseconds(1);
  CAN_Send(40, 1, -k_ang[2], pwm_set);
  delayMicroseconds(1);
  CAN_Send(41, 1, h_ang[2], pwm_set);
  delayMicroseconds(1);
  CAN_Send(30, 1, -k_ang[3], pwm_set);
  delayMicroseconds(1);
  CAN_Send(31, 1, h_ang[3], pwm_set);
/*
