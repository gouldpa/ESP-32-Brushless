...
double Left_per = 1.0;
double Right_per = 1.0;
double Stride_length_a[LC];
double Stride_offset = 0.0;

double TOG_Base = 0.2;
double Base_Height_Base = 215.0;
double Foot_Off_Ground_Base = 50.0;
double Stride_length_Base = 70.0;
double Left_per_Base = 1.0;
double Right_per_Base = 1.0;
double Stride_length_a_Base[LC];
double Stride_offset_Base = 0.0;

double TOG_Targ = 0.2;
double Base_Height_Targ = 215.0;
double Foot_Off_Ground_Targ = 40.0;
double Stride_length_Targ = 0.0;
double Left_per_Targ = 1.0;
double Right_per_Targ = 1.0;
double Stride_length_a_Targ[LC];
double Stride_offset_Targ = 0.0;

double TOG_Delta = 0.0;
double Base_Height_Delta = 0.0;
double Foot_Off_Ground_Delta = 0.0;
double Stride_length_Delta = 0.0;
double Left_per_Delta = 0.0;
double Right_per_Delta = 0.0;
double Stride_length_a_Delta[LC];
double Stride_offset_Delta = 0.0;

int transition;
#define TRANS_MAX 1000
...
void Trans_Start()
{
  transition = TRANS_MAX;
	Stride_length_Delta = (Stride_length - Stride_length_Targ) / TRANS_MAX;
}
void Steer_Trans_Start()
{
  steer_trans = STEER_MAX;
  Left_per_Delta = (Left_per - Left_per_Targ) / STEER_MAX;
  Right_per_Delta = (Right_per - Right_per_Targ) / STEER_MAX;
}
void Trans_Steer() {
	if (steer_trans > 0)
	{
		steer_trans--;
		Left_per -= Left_per_Delta;
    Right_per -= Right_per_Delta;
		Serial.print(steer_trans);
		Serial.print(" ");
		Serial.println(Left_per);
    Serial.print(" ");
    Serial.println(Right_per);
	}
}
...
void Trans() {
  ...
  {
    transition--;
    Stride_length -= Stride_length_Delta;
...
void walk()
{
    ...
    Stride_length_a[0] = Stride_length*Left_per;
    Stride_length_a[1] = Stride_length*Right_per;
    Stride_length_a[2] = Stride_length*Left_per;
    Stride_length_a[3] = Stride_length*Right_per;
...
if (mov || transition || walk_counter )
{
  walk_cnt_max = sp;
    walk_counter++;
...
//delay(5);
  ...
  //  c = Serial.read();
    ...
    else if (c=='p')
    {
      ...
      CAN_Send(41, 1, 2000, pwm_set);
      CAN_Send(42, 1, -12000-TILT, pwm_set);
      CAN_Send(30, 1, 0, pwm_set);
      CAN_Send(31, 1, -2000, pwm_set);
      CAN_Send(32, 1, 12000-TILT, pwm_set);
      CAN_Send(20, 1, 0, pwm_set);
      CAN_Send(21, 1, -2000, pwm_set);
      CAN_Send(22, 1, -12000-TILT, pwm_set);
      CAN_Send(10, 1, 0, pwm_set);
      CAN_Send(11, 1, 2000, pwm_set);
      CAN_Send(12, 1, 12000-TILT, pwm_set);
      SerialBT.println('p');
    }
        else if (c=='o')
    {
      ...
      CAN_Send(40, 1, 0, pwm_set);
      CAN_Send(41, 1, -2000, pwm_set);
      CAN_Send(42, 1, -12000, pwm_set);
      CAN_Send(30, 1, 0, pwm_set);
      CAN_Send(31, 1, 2000, pwm_set);
      CAN_Send(32, 1, 12000, pwm_set);
      CAN_Send(20, 1, 0, pwm_set);
      CAN_Send(21, 1, 2000, pwm_set);
      CAN_Send(22, 1, -12000, pwm_set);
      CAN_Send(10, 1, 0, pwm_set);
      CAN_Send(11, 1, -2000, pwm_set);
      CAN_Send(12, 1, 12000, pwm_set);
      SerialBT.println('o');
    }
    else if (c=='s')
    {
      //mov=1;
      CAN_Send(40, 1, -10000, pwm_set);
      CAN_Send(41, 1, 7000, pwm_set);
      CAN_Send(42, 1, -12000, pwm_set);
      CAN_Send(30, 1, 10000, pwm_set);
      CAN_Send(31, 1, -7000, pwm_set);
      CAN_Send(32, 1, 12000, pwm_set);
      CAN_Send(20, 1, -10000, pwm_set);
      CAN_Send(21, 1, -7000, pwm_set);
      CAN_Send(22, 1, -12000, pwm_set);
      CAN_Send(10, 1, 10000, pwm_set);
      CAN_Send(11, 1, 7000, pwm_set);
      CAN_Send(12, 1, 12000, pwm_set);
      SerialBT.println('s');
    }
        else if (c=='d')
    ...
    {
      mov=1;
	    Stride_length_Targ = Stride_length_Base;
	    Trans_Start();
